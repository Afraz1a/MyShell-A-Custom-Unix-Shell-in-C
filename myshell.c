...
typedef struct Command {
    ...
    char *output_file;
    int append;
    /* new: stderr redirection support */
    char *stderr_file;
    int stderr_append;
    int stderr_to_stdout; /* if 1 then redirect stderr to stdout */
    struct Command *next;
} Command;
...
/* ------------------ scheduling sims and VFS unchanged ------------------ */
...
/* ------------------ job control + history + aliases unchanged ------------------ */
...
/* ------------------ input reader unchanged ------------------ */
static char* read_input(void) {
    enable_raw_mode();

    char *buf = malloc(MAX_LINE);
    if (!buf) return NULL;
    int pos = 0, history_idx = history_count;

    while (1) {
        char c;
        if (read(STDIN_FILENO, &c, 1) != 1) continue;

        if (c == '\n') { buf[pos] = '\0'; write(STDOUT_FILENO, "\n", 1); break; }

        else if (c == 127) { // Backspace
            if (pos > 0) { pos--; write(STDOUT_FILENO, "\b \b", 3); }
        }
...
/* ------------------ tokenization: now recognizes '2>' '2>>' and '2>&1' ------------------ */
...
static char **tokenize_line(char *line, int *tok_count_out) {
    ...
    while (*p) {
        ...
        /* handle file descriptor redirection like 2> 2>> 2>&1 */
        else if (isdigit((unsigned char)*p) && *(p+1) == '>') {
            if (*(p+2) == '>' ) {
                tokens[tcount++] = strndup(p, 3); /* e.g. 2>> */
                p += 3;
            } else if (*(p+2) == '&') {
                /* e.g. 2>&1 or 2>&- */
                char buf[6] = {0};
                int i = 0;
                buf[i++] = *p; buf[i++] = '>'; buf[i++] = '&'; p += 3;
                while (*p && !isspace((unsigned char)*p) && i+1 < (int)sizeof(buf)) buf[i++] = *p++;
                buf[i] = '\0';
                tokens[tcount++] = strdup(buf);
            } else {
                tokens[tcount++] = strndup(p, 2); /* e.g. 2> */
                p += 2;
            }
        }
        else if (*p == '>' || *p == '<' || *p == '|') {
...
/* ------------------ parsing: now supports stderr redirections like 2>, 2>>, 2>&1 ------------------ */
static Command* parse_input(const char *rawline) {
    ...
    while (i < tcount) {
        if (!cur) {
            ...
            cur->input_file = NULL; cur->output_file = NULL; cur->append = 0;
            cur->stderr_file = NULL; cur->stderr_append = 0; cur->stderr_to_stdout = 0;
            cur->next = NULL;
            if (!head) head = cur;
            else {
                Command *tmp = head;
        ...
        if (strcmp(tok, "|") == 0) {
            ...
            ++i; continue;
        } else if (strncmp(tok, "2>", 2) == 0 || strncmp(tok, "1>", 2) == 0 || strncmp(tok, "3>", 2) == 0) {
            /* handle fdN>file or fdN>>file or fdN>&M */
            if (tok[2] == '>') { /* like 2>> */
                ++i;
                if (i >= tcount) { fprintf(stderr,"syntax error after '%s'\n", tok); break; }
                free(cur->stderr_file);
                cur->stderr_file = strdup(tokens[i]);
                cur->stderr_append = 1;
                ++i; continue;
            } else if (tok[2] == '&') {
                /* e.g. 2>&1 or 2>&- */
                if (strcmp(tok, "2>&1") == 0) { cur->stderr_to_stdout = 1; }
...
/* ------------------ helper: interpreter/compile handler ------------------ */
static int try_handle_interpreter_or_compile(Command *cmd) {
    if (!cmd || !cmd->name) return 0;
    /* if name contains a dot and file exists, try to handle by extension */
    if (strchr(cmd->name, '.') == NULL) return 0;
    if (access(cmd->name, F_OK) != 0) return 0; /* file must exist */

    const char *ext = strrchr(cmd->name, '.');
    if (!ext) return 0;

    if (strcmp(ext, ".py") == 0) {
        /* python script: exec python3 script [args...] */
        char *argv[MAX_ARGS];
        int idx = 0;
        argv[idx++] = "python3";
        argv[idx++] = cmd->name;
        for (int i = 1; cmd->args[i] && idx < MAX_ARGS-1; ++i) argv[idx++] = cmd->args[i];
        argv[idx] = NULL;
        execvp("python3", argv);
        return -1;
    }
    ...
    if (strcmp(ext, ".c") == 0 || strcmp(ext, ".cpp") == 0) {
        /* compile to temp binary and execute it */
        char outpath[PATH_BUF];
        snprintf(outpath, sizeof(outpath), "/tmp/mysh_exec_%d", (int)getpid());
        const char *compiler = (strcmp(ext, ".c") == 0) ? "gcc" : "g++";
        char cmdline[PATH_BUF*2];
        snprintf(cmdline, sizeof(cmdline), "%s -O2 -o %s %s 2> /dev/null", compiler, outpath, cmd->name);
    ...
    if (strcmp(ext, ".java") == 0) {
        /* for java, attempt to run: javac file && java ClassName */
        char classbuf[PATH_BUF];
        strncpy(classbuf, cmd->name, sizeof(classbuf)-1);
        char *dot = strrchr(classbuf, '.'); if (dot) *dot = '\0';
        char compile_cmd[PATH_BUF*2];
        snprintf(compile_cmd, sizeof(compile_cmd), "javac %s 2> /dev/null", cmd->name);
        int rc = system(compile_cmd);
        if (rc != 0) { fprintf(stderr, "javac error\n"); _exit(127); }
        char *argv[MAX_ARGS];
        int idx = 0;
        argv[idx++] = "java";
        argv[idx++] = classbuf;
    ...
    /* unknown extension: fall back to exec if executable */
    return 0;
}

/* ------------------ execution: improved pipeline job control and redirections ------------------ */
static int execute_pipeline(Command *cmd_list, int background, const char *full_line) {
    ...
    /* We'll store all child pids so parent can wait for the entire group */
    pid_t child_pids[64]; int child_count = 0;
    ...
    while (cmd) {
        ...
        if (pid == 0) {
            ...
            /* output redirection takes precedence over piping for this command */
            if (cmd->output_file) {
            ...
            /* stderr handling: either redirect to file or to stdout */
            if (cmd->stderr_to_stdout) {
                ...
                if (fd < 0) { perror("open stderr"); /* continue but warn */ }
                else { dup2(fd, STDERR_FILENO); close(fd); }
            }

            /* close any leftover fds in child */
            if (pipefd[0] != -1) close(pipefd[0]);
            /* handle interpreter / compile cases first */
            if (try_handle_interpreter_or_compile(cmd) == -1) _exit(127);
    ...
    if (!background) {
        ...
        do {
            ...
            if (WIFSTOPPED(status)) {
                /* find job leader and add job */
                add_job(pgid, full_line, STOPPED);
                printf("\n[%d] Stopped\n", next_job_id - 1);
                break;
            }
...
static int execute_command(Command *cmd_list, int background, const char *full_line) {
    ...
    if (pid == 0) {
        ...
        if (cmd_list->stderr_to_stdout) dup2(STDOUT_FILENO, STDERR_FILENO);
        else if (cmd_list->stderr_file) {
            int flags = O_WRONLY | O_CREAT | (cmd_list->stderr_append ? O_APPEND : O_TRUNC);
            int fd = open(cmd_list->stderr_file, flags, 0644);
            if (fd < 0) { perror("open stderr"); }
            else { dup2(fd, STDERR_FILENO); close(fd); }
        }
        if (try_handle_interpreter_or_compile(cmd_list) == -1) _exit(127);
